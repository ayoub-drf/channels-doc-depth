...
screenshareButton.onclick = async () => {
  if (!statedCallShare) {
    localStream3 = await navigator.mediaDevices.getDisplayMedia({
      video: true,
      audio: {
    ...
    document.getElementById("screenshareButton_img").src =
      "/static/video_p2p/icons/stop_screenShare.png";
...
let StopScreenShare = async () => {
  ...
  document.getElementById("screenshareButton_img").src =
    "/static/video_p2p/icons/screen_share.png";
...
let increatemed = async (data) => {
  ...
  localStream.getTracks().forEach((track) => {
    console.log('Push tracks from local stream to peer connection')
    pc.addTrack(track, localStream);
  });

  // Pull tracks from remote stream, add to video stream
  pc.ontrack = (event) => {
    console.log('when peer connection ontrack add tracks to remoteStream(User 2)')
...
let createCalloffer = async (unid) => {
  ...
  pc.onicecandidate = (event) => {
    console.log("Get candidates for caller, save to db", event.candidate.toJSON()) 
  ...
  console.log("Save sdp offer from the called to db")
